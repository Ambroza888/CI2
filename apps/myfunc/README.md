# myfunc HTTP Google Cloud Function
...
### Nx Commands
    nx lint myfunc
    nx format:write myfunc
    nx format:write  myfunc
    nx format:check  myfunc
    nx affected --target=build
    nx build myfunc --with-deps

### Google Cloud Commands
    gcloud functions deploy myfunc --set-env-vars foo=bar, zoo=lop
    gcloud functions myfunc --update-env-vars foo=bar, zoo=lop
    gcloud functions deploy myfunc --service-account emailOfServiceAccount
    gcloud functions deploy myfunc --max-instances maxInstancesCount
    gcloud functions deploy myfunc --clear-max-instances
